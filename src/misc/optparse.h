...
#ifndef MISC_OPTPARSE_H
#define MISC_OPTPARSE_H
...
static void
optparse_permute(const struct optparse * const options, int index)
{
...
optparse(struct optparse *options, const char *optstring)
{
	...
	/* "--" forces an end of option-scanning */
	if (optparse_is_dashdash(option)) {
		options->optind++; /* consume "--" */
		return -1;
	}

	if (!optparse_is_shortopt(option)) {
		if (options->permute) {
